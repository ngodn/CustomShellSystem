...
Manifest Manifest::parse(const Json& j,const fs::path& dir) {
    if(!j.is_object()) throw std::runtime_error("Extension manifest must be a JSON object");
    Manifest m;
    m.schema=j.value("schema",0);m.api=j.value("api",0);
    // Legacy pairs (1,1) and (1,2) shipped with CSS 0.3.x/0.4.x; (2,3) is standalone CSSX 1.0.
    const bool legacy=m.schema==1 && (m.api==1 || m.api==2), current=m.schema==manifest_schema && m.api==manifest_api;
    if(!legacy && !current) throw std::runtime_error("Unsupported CSSX manifest schema "+std::to_string(m.schema)+"/api "+std::to_string(m.api)+"; this CSSX supports schema 1 (api 1-2) and schema 2 (api 3)");
    ...
    m.description=text(j,"description",2048,false); m.kind=text(j,"kind",16); m.layout=text(j,"layout",16,false);
    if(m.kind!="native" && m.kind!="lua") throw std::runtime_error("Extension kind must be native or lua");
    // Layout is a legacy hint (the standalone UI has one layout). Accept the old values, default the new schema.
    if(m.layout.empty()) m.layout="tabs";
...
#ifdef _WIN32
    const bool native_ok=suffix==".dll";
#else
    const bool native_ok=suffix==".dll" || suffix==".so";   // host tests load Linux fixtures
#endif
    if((m.kind=="native" && !native_ok) || (m.kind=="lua" && suffix!=".lua")) throw std::runtime_error("Extension entry does not match its kind");
...
Json Manifest::json() const {
    return {{"id",id},{"schema",schema},{"api",api},{"title",title},{"version",version},{"author",author},{"description",description},{"kind",kind},{"layout",layout},{"banner",path_utf8(banner)}};
}
...
void validate_model(const Json& j) {
    ...
    for(const auto& section:j["sections"]) {
        auto id=text(section,"id",96); text(section,"title",96); text(section,"description",1024,false);
        ...
        for(const auto& c:section["controls"]) {
            ...
            // Schema 2 additions. hint: one short line under the value; unit: value suffix;
            // severity: how the UI marks the control (info, warning, danger); effect: what an
            // action does to the game (reversible, persistent, irreversible) for the Cheat Menu rule.
            if(c.contains("hint")) text(c,"hint",256,false);
            if(c.contains("unit")) text(c,"unit",16,false);
            if(c.contains("severity")) { const auto s=text(c,"severity",16); if(s!="info" && s!="warning" && s!="danger") throw std::runtime_error("Unsupported control severity"); }
            if(c.contains("effect")) { const auto s=text(c,"effect",16); if(s!="reversible" && s!="persistent" && s!="irreversible") throw std::runtime_error("Unsupported control effect"); }
...
Json bind_menu(const Json& definition,const Json& model) {
    const int schema=definition.is_object()?definition.value("schema",0):0;
    if(schema!=1 && schema!=2) throw std::runtime_error("Unsupported CSSX UI schema; expected 1 or 2");
