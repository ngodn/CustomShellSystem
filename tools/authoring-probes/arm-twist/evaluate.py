...
new_bind_mesh=unreal.load_asset('/Game/CSSAuthoring/DiagnosticReferences/SK_ArmTwistV44A_V1')
...
shared=unreal.AssetToolsHelpers.get_asset_tools().duplicate_asset('SKEL_TwistProbeCSS','/Game/CSSAuthoring/DiagnosticReferences',shared)
assert shared
...
expected=json.loads((MOD/'work/CSS_SeduXtress_ArmTwistV44A.bindpose.json').read_text())
...
for policy,skel in [('css',shared),('game_rotations',rotations)]:
 ...
 for label,src,bind in [('baseline',original,old_bind),('candidate',new_bind_mesh,new_bind)]:
  name=label+'_'+policy
  mesh=unreal.AssetToolsHelpers.get_asset_tools().duplicate_asset('SK_TwistProbe_'+name,'/Game/CSSAuthoring/DiagnosticReferences',src)
...
animation=unreal.AssetToolsHelpers.get_asset_tools().create_asset('AN_ArmTwistH2Samples_V1','/Game/CSSAuthoring/TransientProbes',unreal.AnimSequence,factory);assert animation
...
def snapshot(pose,label):
 ...
 return dict(bIsValid=True,SkeletalMeshName='SK_Sester_Genessa_V6' if label=='control' else 'SK_ArmTwistV44A_V1' if label.startswith('candidate') else 'SK_SeduXtress_HandBindV43',BoneNames=names,LocalTransforms=transforms)
...
unreal.log('CSS_ARM_TWIST_CANDIDATE_PROBE_PASSED')
