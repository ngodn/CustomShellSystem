"""Verify full B2 animation input against retained arm and hand evidence.

Run with UE5.6.1 PythonScript and CSS_B2_BINDING_DIR set to a fresh work child.
No assets are saved. This covers sampled H2 retargeting and hand IK, not the
entire player AnimGraph or the eventual production Skeleton metadata.
"""
import json, math, hashlib
from pathlib import Path
import unreal
import os
OUT = Path(os.environ['CSS_B2_BINDING_DIR']).resolve()
...
new_bind_mesh=unreal.load_asset('/Game/CSSAuthoring/DiagnosticReferences/SK_ArmRestV44B2ImportedMorphs_V1')
shared=unreal.load_asset('/Game/CSSAuthoring/Shared/Skeletons/SKEL_CSS_Base')
rotation_package=os.environ.get('CSS_B2_REFERENCE_SKELETON','/Game/CSSAuthoring/DiagnosticReferences/SKEL_CSSGameRotations_V1')
assert rotation_package.startswith('/Game/CSSAuthoring/DiagnosticReferences/')
rotations=unreal.load_asset(rotation_package)
assert all((human,control,original,new_bind_mesh,shared,rotations))
shared=unreal.AssetToolsHelpers.get_asset_tools().duplicate_asset('SKEL_B2InputProbeCSS_V1','/Game/CSSAuthoring/DiagnosticReferences',shared)
assert shared
for asset in (human,control,original,new_bind_mesh,rotations):
 p=production.parent/(asset.get_path_name().split('.')[0].removeprefix('/Game/')+'.uasset')
...
expected=json.loads((MOD/'work/CSS_SeduXtress_ArmRestV44B2.bindpose.json').read_text())
...
for policy,skel in [('css',shared),('game_rotations',rotations)]:
 ...
 for label,src,bind in [('candidate',new_bind_mesh,new_bind)]:
  name=label+'_'+policy
  mesh=unreal.AssetToolsHelpers.get_asset_tools().duplicate_asset('SK_B2InputProbeV1_'+name,'/Game/CSSAuthoring/DiagnosticReferences',src)
...
hand_names=[m['bone'] for m in json.loads((Path(__file__).parent/'left-finger-calibration-v1.json').read_text())['parameters']]
attachment_names={'Socket_Prop_InHand_Hook_R','Socket_Prop_InHand_Lowerarm_L_01','Socket_Prop_InHand_Hook_L','Socket_Prop_Upperarm_L_01','Socket_ShoulderBash'}
comparisons=[]
def rotation_error(a,b):
 a=[a[k] for k in 'XYZW'];b=[b[k] for k in 'XYZW']
...
animation=unreal.AssetToolsHelpers.get_asset_tools().create_asset('AN_B2InputH2Samples_V1','/Game/CSSAuthoring/TransientProbes',unreal.AnimSequence,factory);assert animation
...
def snapshot(pose,label):
 ...
 return dict(bIsValid=True,SkeletalMeshName='SK_Sester_Genessa_V6' if label=='control' else 'SK_ArmRestV44B2ImportedMorphs_V1' if label.startswith('candidate') else 'SK_SeduXtress_HandBindV43',BoneNames=names,LocalTransforms=transforms)
 ...
 for label,mesh in meshes:
  ...
  for sample in range(5):
   ...
   for alpha in (0.,1.):
    ...
    previous=json.loads((OUT.parent/'arm-rest-alignment-v1'/(name+'.json')).read_text())['pose']['Snapshot']
    assert after['BoneNames']==previous['BoneNames']
    max_angle=max_position=0.
    for bone,actual,expected_pose in zip(after['BoneNames'],after['LocalTransforms'],previous['LocalTransforms'],strict=True):
     if bone in attachment_names or bone.startswith('VB '):continue
     angle=rotation_error(actual['Rotation'],expected_pose['Rotation'])
     distance=math.dist(list(actual['Translation'].values()),list(expected_pose['Translation'].values()))
     max_angle=max(max_angle,angle);max_position=max(max_position,distance)
     assert angle<.001 and distance<.001,(name,bone,angle,distance)
    comparisons.append(dict(case=name,maximum_rotation_degrees=max_angle,maximum_translation_cm=max_position))
    if label=='candidate_game_rotations' and compressed and alpha==1.:
     anchor=json.loads((OUT.parent/'hand-combined-fixtures-v1'/f'calibration-h2-{sample}.json').read_text())['pose']['Snapshot']
     for bone in hand_names:
      actual=after['LocalTransforms'][after['BoneNames'].index(bone)]
      expected_pose=anchor['LocalTransforms'][anchor['BoneNames'].index(bone)]
      assert rotation_error(actual['Rotation'],expected_pose['Rotation'])<.001,(sample,bone,'hand input convention')
assert all(hashlib.sha256(Path(p).read_bytes()).hexdigest()==h for p,h in hashes.items())
contrasts=[]
for sample in range(5):
 a=json.loads((OUT/f'candidate_css-s{sample}-compressed-ik1.json').read_text())['pose']['Snapshot']
 b=json.loads((OUT/f'candidate_game_rotations-s{sample}-compressed-ik1.json').read_text())['pose']['Snapshot']
 errors=[rotation_error(a['LocalTransforms'][a['BoneNames'].index(n)]['Rotation'],b['LocalTransforms'][b['BoneNames'].index(n)]['Rotation']) for n in hand_names]
 assert max(errors)>30.,(sample,'negative control does not separate reference conventions')
 contrasts.append(dict(sample=sample,maximum_old_reference_difference_degrees=max(errors)))
(OUT/'evaluation.json').write_text(json.dumps(dict(passed=True,production_unchanged=True,production_hashes=hashes,results=results,reference_comparisons=comparisons,full_b2_bind_verified=True,rotation_package=rotation_package,input_convention_contrasts=contrasts,raw_hand_convention_verified_samples=5,scope=__doc__),indent=2)+'\n')
unreal.log('CSS_B2_ANIMATION_BINDING_PASSED')
