...
## Native Cheat Menu drafts, recovery and pickups

The Cheat Menu now keeps draft values separate from applied settings. Settings contains Apply settings, Discard changes and Turn off all cheats. Numeric settings are saved only on Apply. A failed save restores the previous active settings; failed cleanup stops periodic work and remains retryable. Controller changes stop cheats, and pawn replacement restores old CharacterData before acquiring another baseline. Confirmation messages include the current resource amount, pickup or shell target. The shared UI supports short disabled labels and native action-error text.

The C++ completed-intro observer matches the original PrologueRecovery.lua predicate and nine-observation delay. Portable tests cover successful cleanup, no repeat calls, unfinished intros, animation activity, duplicate instances, extra tag counts and changed controllers. The live normal-state check reported that the known lock was absent and left gameplay unchanged. A live positive lock is not currently available.

Pickup catalog enumeration returned 77 entries. The native item tools provide search, an applied quantity, add, remove and confirmed give-all. Engine requests now include table.rows and a typed $table_field argument reference. The latter resolves the source table and row again and checks the reflected type before copying it into a function frame. Trollweed resolved through the real 40-byte soft-class parameter; a mismatched soft-class-to-integer argument was rejected before the call. The UE4SS object setter wrapper was unavailable on this build, so ordinary object parameters use the same checked CopyCompleteValue path as the existing Call helper. No item grant/removal was performed during these read-only runtime probes.

Full feature status is tracked in cheat-menu-parity.md. This remains a development build, not a completed port or a 0.3.0 release.
