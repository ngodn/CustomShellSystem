...
if os.environ.get('CSS_B2_CLEARANCE_SYNTHETIC')=='1':
    for c in load(AUDIT/'left-graph-anchor-animation-articulated-v2/manifest.json')['cases']:
        cases.append(dict(group='synthetic',label=c['pose'],source=AUDIT/'left-graph-anchor-animation-articulated-v2'/c['pose']))
for c in load(AUDIT/'left-observed-graph-articulated-v1/manifest.json')['cases']:
...
(out/'report.json').write_text(json.dumps(dict(scope=__doc__,passed=passed,samples=rows,groups=groups,seconds=time.monotonic()-started),indent=2)+'\n')
