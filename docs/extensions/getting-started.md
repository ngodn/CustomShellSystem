...
`describe` includes an `enum` list of `{name, value}` entries for reflected enum parameters. Validate named choices and exclude terminal `MAX` entries rather than assuming integer values survive a game update. `properties` lists an object's declared property names, element sizes and array dimensions; pass `inherited: true` to include base classes. Both operations are read-only and bounded. Metadata describes layout, not whether a gameplay function is safe to call.
