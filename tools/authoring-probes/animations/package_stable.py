...
base = ROOT/'work/anim13/base/decoded'
stage = OUT/'stage'
...
variant['animations'] = {slot.lower(): [dict(id='eve', name='Eve (Stellar Blade)',
    blend_space='/Game/CSS/Eve/Anim/BS_S1_'+slot+'.BS_S1_'+slot)] for slot in ('Walk','Jog','Sprint')}
for suffix in ('.utoc','.ucas'):
    p = trio/(stem+suffix)
...
reversed_manifest['version'] = old['version']
...
(OUT/'verification.json').write_text(json.dumps(dict(passed=True, trio=str(trio),
    ...
    added_animation_packages=packages, metadata_change='Only alpha version, movement definitions and container checksums',
