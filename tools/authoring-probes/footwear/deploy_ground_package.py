"""Install accepted Black Pearl grounding with a normal restart. Python 3.14."""
import json
...
out=CSS/'work/ground1/live';assert not out.exists()
...
candidate=mod/'work/ground1/trio'
prior=load(CSS/'work/ui-live1/deployment.json')
...
assert load(mod/'work/ground1/verification.json')['passed']
assert hashes(candidate)==load(mod/'work/ground1/verification.json')['hashes']
...
for name in ('host','tests','windows','shipping'):
    assert load(CSS/f'work/ground1/build/{name}-exit.json')['exit_code']==0
assert len(processes())==1
name='css_core-ground1.dll';assert not (runtime/'cores'/name).exists()
...
print('Grounding core and package installed; normal Steam launch requested.')
