...
Selected upload: [css-nexus-header-1300x372-transparent.png](v0.1.1/css-nexus-header-1300x372-transparent.png).
The PNG is exactly **1300 x 372 pixels**, 694,165 bytes, with RGBA transparency.

## Transparent canvas export

At the user's request, the complete version 2 artwork is fitted proportionally
inside a transparent 1300 x 372 canvas. No crop, stretch or regeneration is applied.
The artwork occupies 1008 x 372 pixels at (146, 0), leaving 146 transparent
pixels on both sides. The original generated master remains unchanged.

Python 3.14.7 and Pillow 12.3.0 were used:

```python
fitted = ImageOps.contain(source.convert("RGBA"), (1300, 372),
                         method=Image.Resampling.LANCZOS)
canvas = Image.new("RGBA", (1300, 372), (0, 0, 0, 0))
canvas.alpha_composite(fitted, (146, 0))
```

The saved dimensions, RGBA mode and transparent side pixels were verified.
The earlier cropped export is superseded and should not be uploaded.
...
for history. Use `css-nexus-header-1300x372-transparent.png` for the Nexus header.
...
Canvas: 1300 x 372 pixels, very wide 3.495:1 banner. Fill the whole canvas, no outside padding. Preserve ample safe margins for the title.
Reference 1: original wardrobe-v1.png is the exact CSS visual identity: antique gold serif typography, moth and armored shell crest, delicate thorn-like gold border, black textured surface, author credit.
Reference 2: BeauteGenessa gameplay wardrobe screenshot provides the hooded pale-faced character identity and dark fitted costume.
Reference 3: Seductress inventory polearm screenshot provides the pale face, glowing eyes, ornate horn-like headpiece, crimson and antique gold outfit detail and attractive cinematic lighting.
Reference 4: Colors Crimson screenshot provides the crimson/gold palette and outfit material reference; do not reproduce sliders or menu controls.
Reference 5: Seductress side gameplay screenshot provides environmental atmosphere and realistic material detail.
Composition: beautifully art-directed horizontal dark-fantasy banner. A faithful hooded Genessa portrait emerges at the left edge, a faithful Seductress head-and-shoulders portrait with horned headpiece emerges at the right edge. Portraits occupy the outer quarters; focus on faces and armor detail, not breasts. The central half is calm obsidian black with subtle smoky teal depth, clear strong typography, and the small original moth/armor crest above the title. Integrate the portraits seamlessly into a dark stone and mist backdrop inspired by the screenshots. Fine antique gold border and restrained corner ornaments; sophisticated, legible, atmospheric. No crowded collage or separate rectangular panels.
Text exactly: small spaced uppercase "CUSTOM", beneath it the dominant title "SHELL SYSTEM", beneath that modest "by _eins0fx". The leading underscore in _eins0fx is essential. All typography in luminous aged gold, matching reference 1. Title must be readable at the final banner size. No additional text, game name, version, buttons, minimap, HUD, screenshot notifications, weapon lists, FPS counters, logos from other games, or watermark.
Maintain the supplied characters' faces, distinctive headwear and outfit identity. Realistic rendered game-character look, elegant chiaroscuro with warm rim light and subdued cool shadows, no plastic skin, no illustration/anime restyling. This banner should feel like the same product as wardrobe-v1.png.
